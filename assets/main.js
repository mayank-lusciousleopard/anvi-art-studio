...
setTimeout(function(){
    ...
    $(document).ready( function() {
        ...
        function autoPlayVideos(){
          if (document.querySelector('.videos-autoplay')  ) {
            setTimeout(function(){
                if (document.querySelector('.gallary-item video')) {
                    const player = new Plyr(document.querySelector('.gallary-item video'));
                    window.player = player;
                    player.play(); 
                }
                if (document.querySelector('.swiper-slide-active video')) {
                  const player = new Plyr(document.querySelector('.swiper-slide-active video'));
                  window.player = player;
                  player.play(); 
                }
            },250)
        ...
        function SectionLoadGroupProduct() {
            ...
            if ($(".group-product-main").length > 0) {
                ...
                $('.subtotal span').text(Shopify.formatMoney(subtotalOnload));
            }
            // endd
        }
        ...
        function SectionLoadBoughtTogether() {
            ...
            if ($(".bought-together").length > 0) {
                ...
                $('.final-price span').attr("data-subtotal", Shopify.formatMoney(finalPrice));
                $('.final-price span').text(Shopify.formatMoney(finalPrice));
                $('.different-price').text(Shopify.formatMoney(( subtotalOnload - finalPrice)));
                $('.discount-price span > s').attr("data-discountTotal", Shopify.formatMoney(subtotalOnload));
                $('.discount-price span > s').text(Shopify.formatMoney(subtotalOnload));
            }
        ...
        window.ReloadYoptoReviews = function ReloadYoptoReviews() {
            setTimeout(function(){
                if (document.querySelector('.yotpo-stars')) {
                   var ReloadYoptoReviews = new Yotpo.API(yotpo);
                    ReloadYoptoReviews.refreshWidgets();
                }
            },500)
        }
            ...
            $(section.querySelectorAll('.countdown')).each( function() {
              if ($(this).attr("data-enddate")) {
                ...
                _this.countdown( endDate ).on( 'update.countdown', function ( event ) {
                    ...
                    _this.closest('.product-deal-wrap').addClass('d-none');
                    _this.closest('.product-deal-wrap, .product-deal, .product-deal-wrap').addClass('d-none');
                });
              }
            }); 
        }
        ...
        window.cartCount = function cartCount(parsedState) {
            ...
            var cart_total = parsedState.items_subtotal_price;
                    ...
                    $.getJSON(`${window.Shopify.routes.root}search/suggest.json`, {
                        ...
                        if (products.length > 0) {
                            $.each(products, function(index, item) {
                                const imgHtml = (item.featured_image.url != null && item.featured_image.url != '') ? '<span class="img"><img src="' + item.featured_image.url + '" loading="lazy" width="'+  productImgWidth +'" height="'+ productImgHeight +'"/></span>' : '';
                                ...
                                function setInputAvailability(listOfOptions, listOfAvailableOptions) {
                                    listOfOptions.forEach((input) => {
                                      if (listOfAvailableOptions.includes(input.getAttribute('value'))) {
                                        $(input).trigger('click')
                                        input.classList.remove('unavailable-opt')
                                      } else {
                                        input.classList.add('unavailable-opt')
                                      }
                                    });
                                  }
                            },
        ...
        if($('body').find('.lable-with-open-menu').length > 0){
            $(document).on('click','.nav-link, .child-link-title', function(e) {
                if ($(window).width() < 1200) {
                  if ($(this).closest(".nav-item ").find('.sub-menu').length > 0) {
                      e.preventDefault();
                  }
        ...
        function initRecentBought(){
            if (recentBoughtSelector && window?.RecentBoughtJsonConfig?.products && window.RecentBoughtJsonConfig.products.length) {
                recentBoughtTimeout = setTimeout(function() {
                    ...
                    productText = recentBought.p_text,
                    productFrom = recentBought.p_fromtext,
                    productLoc = recentBought.p_loc[Math.floor(Math.random() * recentBought.p_loc.length)].trim(),
                    fromText = window.RecentBoughtJsonConfig.canShowTime == 'true' ? '<span>' + Math.floor((Math.random() * 60) + 1) + '</span> '+ productText + ' ' + productLoc : productFrom + ' ' + productLoc;
        ...
        function applyVariantChangeEvent(productVariant) {
            $('.variant-option', productVariant).on('click', function(event) {
                ...
                if (price && compare_price && selectedVariant.price < selectedVariant.compare_at_price) {
                 ...
                 if (selectedVariant.featured_media != undefined ) {
    ...
    var productGridElement = $(_this).closest('.featured-product-tab-main').find('.collection-grid.active .product-grid')
      if ($(productGridElement).is(':empty')){
        jQuery.ajax({
          type: 'get',
          url: URL,
          dataType: 'json',
          ...
          success: function(response){
            ...
            Promise.all(requests).then(function (responses) {
                ...
                if ($(productGridElement).is(':empty')){
                  productGridElement.html(CollectionTabCard)
                  CardBox()
                }
    ...
    function CollectionAjexTab() {
      if($('body').find('.tab-title').length > 0 && $('.nav-tabs').attr('data-ajex') != undefined){ 
        var collectiontabItem = document.querySelectorAll('.featured-product-tab-main')
        collectiontabItem.forEach(function(selectItem, selectItemIndex){
            var _this = selectItem.querySelector('.tab-title .collection-title.active');
            document.querySelector('body > .loading-box').classList.add('hidden');
            if($(_this).hasClass("active")){
    ...
    function CardBox() {
      ...
      document.querySelectorAll(".product-card-variant-option").forEach((e => {
        e.addEventListener("change", (e => {
          ...
          variantCompareParice = selectedElement.options[selectedElement.selectedIndex].getAttribute("data-compare-price"),
          ...
          ComparePriceElement = productItem.querySelector(".price-item.old-price "),
          ...
          ComparePriceElement.innerHTML = '<s>'+ variantCompareParice+ '</s>';
